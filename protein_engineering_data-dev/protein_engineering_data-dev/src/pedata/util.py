...
def get_target(
    ...
    as_dict: bool = False,
) -> Union[dict[Sequence[str], np.ndarray], tuple[Sequence[str], np.ndarray]]:
    ...
    Args:
        dataset (ds.Dataset): The dataset to extract target variables from.
        max_targets (int, optional): Maximum number of targets to return. Defaults to None, which returns all targets.
        normalization (Callable[[np.ndarray], np.ndarray], optional): A function for normalizing the target values. Defaults to the identity function (no normalization).
        as_dict (bool, optional): Whether to return the targets as a dictionary of target names mapping to values. Defaults to False, in which case target names and values are returned as a tuple.
...
def adjust_all_targets_maximization(
    ...
    Args:
        d (ds.Dataset): The dataset to adjust.
...
def zscore(
    ...
    Args:
        ...
        mean (np.ndarray): Should be set as an input when the value is already known. Typically for transforming a test set/target
        std (np.ndarray): Should be set as an input when the value is already known. Typically for transforming a test set/target
...
def de_zscore_predictions(
    ...
    Args:
        zs_pred_means (np.ndarray): The input mean array to be de_zscored.
        zs_pred_stds (np.ndarray): The input std array to be de_zscored.
        mean (np.ndarray): the mean from the original (train) array
        std (np.ndarray): the std from the original (train) array
...
def get_summary_variable(
    ...
    Args:
        dataset (ds.Dataset): Dataset to extract summary variable from
        normalization (Callable[[np.ndarray], np.ndarray], optional): A normalization function for the summary variable. Defaults to zscore.
        objectives (OptimizationObjective): The optimization objective for the target variable.
...
def append_summary_variable(
    ...
    Args:
        dataset (ds.Dataset): Dataset to append summary variable to
        normalization (Callable[[np.ndarray], np.ndarray], optional): A normalization function for the summary variable. Defaults to zscore.
        objectives (OptimizationObjective): The optimization objective for the target variable.

    Returns:
        ds.Dataset: Dataset with summary variable appended as a new column.
    ...
    if num_targets < 1:
        ...
        return dataset.add_column(
            ...
            get_summary_variable(dataset, normalization=normalization, objectives=objectives),
        )
...
class DatasetHandler(object):
    ...
    def cat(self, hf_ds: Dataset) -> torch.Tensor:
        ...
        Args:
            hf_ds (Dataset): Dataset to concatenate
    ...
    def get(self, t: torch.Tensor, *feat: list[str]) -> torch.Tensor:
        ...
        Args:
            t (torch.Tensor): Concatenated tensor
            feat (list[str]): List of feature names to extract

        Returns:
            torch.Tensor: Tensor containing the extracted feature(s)
    ...
    def dims(self, feat: list[str]) -> tuple[int]:
        ...
        Args:
            feat (list[str]): List of feature names.

        Returns:
            tuple[int]: Tuple containing the dimensions of the feature(s) in the concatenated tensor.
