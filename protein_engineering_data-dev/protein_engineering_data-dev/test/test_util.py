...
def get_small_large_df() -> tuple[ds.Dataset, ds.Dataset, str]:
    ...
    for df in [large_df, small_df]:
        df.set_format(columns=["aa_seq", "aa_mut"])
...
def test_get_target_columns():
    ...
    dataset = ds.Dataset.from_dict({"aa_mut": ["wt", "A2G"], "aa_seq": ["MAPEKT", None]})
    ...
    assert get_target_columns(dataset) == ["target kcat / kmol"]
...
def test_adjust_all_targets_maximization():
    ...
    adjusted_dataset = adjust_all_targets_maximization(dataset, objectives)
    adjusted_dataset.set_format("numpy")
    ...
    assert np.all(
        adjusted_dataset["target fix"] == -np.abs(np.arange(len(dataset)) - len(dataset))
