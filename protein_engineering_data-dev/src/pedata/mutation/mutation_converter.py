...
| `convert_variant_mutation_to_str`: Mut objects into str 
| `convert_all_variant_mutations_to_str`: Mut objects into list
| `dict_to_namedtuple_mut`: dictionary into Mut objects
| `namedtuple_to_dict_mut`: Mut objects into dictionary """
...
def convert_variant_mutation_to_str(
    ...
    Args:
        mut (list[Mut]): Mutations in the format (position, source, target)
        delimiting_char (str): Character used to connect the mutations (default is "_")
        offset (int): Offset value to adjust mutation positions (default is 0)

    Returns:
        str: Mutation code encoded with delimiting character
    ...
    mut = [mu.convert_tuple_to_valid_namedtuple(m) for m in mut]
...
def convert_all_variant_mutations_to_str(
    ...
    Args:
        mut (sequence[sequence[tuple]]): Sequence of mutation sequences, one for each variant.
        delimiting_char (str): Character used to connect the mutations for one variant (default is "_")
        offset (int): Offset value to adjust mutation positions (default is 0)

    Returns:
        list: Returns all mutations converted to string encodings only when the mutation has valid sequences of namedtuples, otherwise raise ValuesError
    ...
    if not isinstance(mut, list) or len(mut) == 0:
        raise TypeError(f"Invalid input: Expected a list of mutation but got {type(mut)}")
...
def dict_to_namedtuple_mut(mut: list[Dict[str, list]]) -> list[Mut]:
    ...
    Args:
        dicts (List[Dict[str, Any]]): List of dictionaries containing 'position', 'source', and 'target' keys.

    Returns:
        List[Mut]: List of namedtuples 'Mut'.
...
def namedtuple_to_dict_mut(mut_list: list[Mut]) -> list[Dict[str, list]]:
    ...
    Args:
        mut_list (List[Mut]): List of namedtuples 'Mut'.

    Returns:
        Dict[str, list]]: Dictionary containing 'position', 'source', and 'target' keys.
