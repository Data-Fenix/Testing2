from pedata.exazyme_datasets.utils import (
    append_index_column_to_dataset,
    dataset_base_processing,
)
import datasets
import pytest
...
def test_dataset_base_processing(dataset_as_Dataset):
    """Test that the dataset_base_processing function works"""
    # with defaults parameter
    dataset = dataset_base_processing(dataset_as_Dataset)
    print(dataset)  # FIXME Write asserts and test with non defaults parameters


def test_dataset_base_processing_with_incorrect_dataset(dataset_as_DatasetDict):
    """Test that the test_dataset_base_processing returns TypeError if the input is incorrect"""
    with pytest.raises(TypeError):
        _ = dataset_base_processing(dataset_as_DatasetDict)
