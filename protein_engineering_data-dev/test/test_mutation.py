...
class TestMutation:
    ...
    def test_generate_variant_mutation_combinations(self):
        ...
        assert result[0] == [Mut(1, "A", "G"), Mut(2, "C", "T")], f"Test Case 3 failed"
    ...
    def test_get_parent_aa_seq(self):
        ...
        dataset = ds.Dataset.from_dict(ed.aa_example_1)
        ...
        assert ds.Dataset.from_dict(ed.aa_example_1)["aa_seq"][0]
    ...
    def test_get_parent_sketch_from_mutations(self):
        ...
        with pytest.raises(ValueError):
            Mutation.get_parent_sketch_from_mutations([ elem for code in ["M1A","B1C"] for elem in Mutation.parse_variant_mutations(code) 
            ])
    ...
    def test_estimate_offset(self):
        ...
        assert Mutation.estimate_offset(dataset, parent=parent_seq)["offset"][0] == 0

        # Test case 5: DNA dataset missing column starting with target
        dataset = ds.Dataset.from_dict(ed.dna_example_1)
        dataset_with_no_parent = dataset.select(range(1, len(dataset)))
        parent_seq = ed.dna_example_1["dna_seq"][0]
        with pytest.raises(KeyError):
        ...
        dataset = ds.Dataset.from_dict(ed.aa_example_2)
        parent_seq = ed.aa_example_2["aa_seq"][0]
        with pytest.raises(KeyError):
        ...
        parent_seq = "MB"
        dataset = ds.Dataset.from_dict(
            {"aa_mut":  ["M1A","B2C", "M1A_B2C"], "aa_seq": [None]*3, "target foo": [1]*3}
        )
